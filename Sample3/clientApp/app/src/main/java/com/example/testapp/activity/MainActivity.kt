...
import com.example.testapp.myViewModel.BusData
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val adapter = MainCustomAdapter(viewModel)
                        ...
                        override fun onResponse(call: Call<Bus>, response: Response<Bus>) {
                            if (response.isSuccessful) {
                                ...
                                if (response.body()?.msgBody?.itemList != null) {
                                    ...
                                    //val routeId1 = response.body()!!.msgBody.itemList[1].busRouteId

                                    val routeIdList = ArrayList<String>()
                                    val routeNmList = ArrayList<String>()

                                    for (i in 0..response.body()!!.msgBody.itemList.size-1) {
                                        routeIdList.add(body.itemList[i].busRouteId) // 노선 ID
                                        routeNmList.add(body.itemList[i].busRouteNm) // 버스 번호 (노선명)
                                    }

                                    //println("첫번째 아이디는 $routeId")
                                    //println("두번째 아이디는 $routeId1")
                                    //println("아이디 개수는 ${response.body()!!.msgBody.itemList.size}") // 12번은 총 56개
                                    for (i in 0..body.itemList.size-1) {
                                        val routeNm = routeNmList[i]
                                        val routeId = routeIdList[i]
                                        println("$routeNm")
                                        println("$routeId")
                                        viewModel.addItem(BusData(routeNm, routeId))
                                    }
                                }
                            }
                        }
